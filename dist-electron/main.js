		...
		return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
			...
			if (config.socketPath) {
				...
				if (config.allowedSocketPaths != null) {
					...
					const resolvedSocket = path$2.resolve(config.socketPath);
					if (!allowed.some((entry) => typeof entry === "string" && path$2.resolve(entry) === resolvedSocket)) return reject(new AxiosError(`socketPath "${config.socketPath}" is not permitted by allowedSocketPaths`, AxiosError.ERR_BAD_OPTION_VALUE, config));
				}
...
ipcMain.handle("get-homepage", async () => await activeExt.get_homepage_games());
ipcMain.handle("get-category", async (e, { category, page }) => await activeExt.get_games_by_category(category, page));
ipcMain.handle("search-games", async (e, { query, page }) => await activeExt.search_games(query, page));
ipcMain.handle("get-steam-media", async (e, gameName) => {
	...
	try {
		...
		const searchRes = await axios.get(`https://store.steampowered.com/api/storesearch/?term=${encodeURIComponent(cleanName)}&l=english&cc=US`);
		if (!searchRes.data.items?.length) return null;
		const appId = searchRes.data.items[0].id;
		const result = (await axios.get(`https://store.steampowered.com/api/appdetails?appids=${appId}`)).data[appId].data;
